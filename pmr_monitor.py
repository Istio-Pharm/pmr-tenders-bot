...
try:
    ...
    bot.send_message(CHAT_ID, f"[Ошибка] Ошибка при подключении к Google Sheets:\n{e}")
...
def fetch_pmr_tenders():
    ...
    try:
        ...
        bot.send_message(CHAT_ID, f"[Ошибка] Ошибка чтения таблицы:
{e}")
        return
    ...
    for row in rows:
        ...
        if tender_id not in existing_ids:
            ...
            message = (
                ...
                f"ID: `{tender_id}`\n"
                f"Дата Дата: {announce_date}\n"
                f"Предмет Предмет: {subject}\n"
                f"Сумма Сумма: {price}\n"
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            bot.send_message(CHAT_ID, f"[Ошибка] Ошибка при проверке тендеров:\n{traceback.format_exc()}")
        time.sleep(10)  # временно уменьшаем интервал до 10 сек

...
print("🔧 Скрипт стартовал успешно — начинается мониторинг...")

try:
    bot = telebot.TeleBot(BOT_TOKEN)
    bot.send_message(CHAT_ID, "🤖 Бот стартовал и проверяет тендеры!")
except Exception as e:
    print("❌ Ошибка при инициализации Telegram:", e)
...
try:
    ...
    bot.send_message(CHAT_ID, f"🚨 Ошибка при подключении к Google Sheets:\n{e}")
...
if __name__ == "__main__":
    print("🔄 Вошёл в цикл while True — стартуем fetch_pmr_tenders()...")
    while True:
        try:
            fetch_pmr_tenders()
        except Exception as e:
            print("🚨 Ошибка в цикле мониторинга:")

...
try:
    ...
    bot.send_message(CHAT_ID, "🤖 Бот стартовал и проверяет тендеры (v2)")
...
def fetch_pmr_tenders():
    print("🔍 Запуск fetch_pmr_tenders...")
    ...
    print(f"📄 Найдено строк (тендеров): {len(rows)}")

    try:
        existing_ids = [row[0] for row in worksheet.get_all_values()]
        print(f"📋 Считано ID из таблицы: {len(existing_ids)}")
    except Exception as e:
        print("❌ Ошибка чтения таблицы:", e)
        bot.send_message(CHAT_ID, f"🚨 Ошибка чтения таблицы:
{e}")
        return
    ...
    print(f"✅ Обработка завершена. Добавлено новых записей: {new_counter}")
